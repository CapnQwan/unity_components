...
namespace Assets.Scripts.Utilities.Debugging.Visualization.Internal
{
  public static class DrawMaterials
  {
    ...
    public static void Init()
    {
      ...
      if (unlitMat == null)
      {
        unlitMat = new Material(Shader.Find("DrawShaders/UnlitColorAlpha"));
      }
      if (shadedMat == null)
      {
        shadedMat = new Material(Shader.Find("DrawShaders/Shaded"));
      }
      if (pointMat == null)
      {
        pointMat = new Material(Shader.Find("DrawShaders/UnlitPoint"));
      }
      if (lineMatRoundedEdge == null)
      {
        lineMatRoundedEdge = new Material(Shader.Find("DrawShaders/LineRounded"));
      }
      if (quadMat == null)
      {
        quadMat = new Material(Shader.Find("DrawShaders/Quad"));
      }
      if (lineMat == null)
      {
        lineMat = new Material(Shader.Find("DrawShaders/Line"));
      }
    }
  }
}