using UnityEngine;
using PerlinNoise = Noise.PerlinNoise;
...
public struct PerlinNoiseParameters
{
  ...
  /// <summary>
  /// Initializes a new instance of the <see cref="PerlinNoiseParameters"/> struct with the specified parameters.
  /// </summary>
  ...
  public PerlinNoiseParameters(
      int width = PerlinNoise.DEFAULT_WIDTH,
      int height = PerlinNoise.DEFAULT_HEIGHT,
      int seed = PerlinNoise.DEFAULT_SEED,
      float scale = PerlinNoise.DEFAULT_SCALE,
      float lacunarity = PerlinNoise.DEFAULT_LACUNARITY,
      float persistance = PerlinNoise.DEFAULT_PERSISTANCE,
      int octaves = PerlinNoise.DEFAULT_OCTAVES,
      NormalizeMode normalizeMode = PerlinNoise.DEFAULT_NORMALIZE_MODE,
      Vector2? offset = null)
  {
    this.Width = width;
    this.Height = height;
    ...
    this.Offset = offset ?? PerlinNoise.DEFAULT_OFFSET;
  }
}
